...
public class Graph<T> {
	...
	*/
	...
	public double excentridadNodo(T node) {
		if (a == null) {
			floyd();
		}
		double aux = 0;
		for (int i=0;i<size;i++) {
			if (a[i][getNode(node)] > aux) {
				aux = a[i][getNode(node)];
			}
		}
		return aux;
	}
	
	public double[] excentridadFullNodos() {
		double[] aux = new double[size];
		for (int i=0;i<size;i++) {
			aux[i] = excentridadNodo(nodes[i]);
		}
		return aux;
	}
	
	public boolean nodeIsConexo(T node) {
		recorridoProfundidad(node);
	...
	public boolean isGraphFuertementeConexo() {
		for (int i=0;i<size;i++) {
			if (!nodeIsConexo(nodes[i]))
				return false;
		}
		return true;
	}
